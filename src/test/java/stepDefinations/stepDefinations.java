...
//	Constructor will be called with value of resource which you pass	
		...
		APIResources resourcrAPI =  APIResources.valueOf(resource);
		System.out.println(resourcrAPI.getResource());
		
		
		if (method.equalsIgnoreCase("POST"))
		{
			response = res.when().post(resourcrAPI.getResource());
		}
//				.then().spec(resspec).extract().response();
		
		else if(method.equalsIgnoreCase("GET"))	
			response = res.when().get(resourcrAPI.getResource());
